...
class Transcript(LlmTools):
    ...
    def main(self, userText, config):
        try:
            ...
            client.chat_postMessage(channel=config.slackChannel, text="Getting file ...")
            ...
            if response.status_code == 200:
                if "video/mp4" in response.headers.get("Content-Type", ""):
                    ...
                    client.chat_postMessage(channel=config.slackChannel, text="Transcribing file ...")
                    ...
                    if transcript != None and transcript.summary != None:
                        ...
                        client.chat_postMessage(channel=config.slackChannel, text=slackMessageF)
                    else:
                        client.chat_postMessage(channel=config.slackChannel, text="No summary available")
                else:
